...
function fetch(what, where, recurse) {
    ...
    if (ret && ret.children && ret.children.length > 0)
        return ret.children[0].data;
    else return false;
}
...
RssHandler.prototype.done = function() {
    ...
    var found = getElements(function(value) {
    ...
    if (feedRoot) {
        if (feedRoot.name === "rss") {
            ...
            feed.id = "";
            if ((tmp = fetch("title", feedRoot.children))) feed.title = tmp;
            if ((tmp = fetch("link", feedRoot.children))) feed.link = tmp;
            ...
            if ((tmp = fetch("managingEditor", feedRoot.children)))
                feed.author = tmp;
            feed.items = [];
            getElements("item", feedRoot.children).forEach(function(
                item,
                index,
                list
            ) {
            ...
            feed.type = "atom";
            if ((tmp = fetch("id", feedRoot.children))) feed.id = tmp;
            if ((tmp = fetch("title", feedRoot.children))) feed.title = tmp;
            if (
                (tmp = getElements("link", feedRoot.children, true)) &&
                (tmp = tmp.attribs) &&
                (tmp = tmp.href)
            )
                feed.link = tmp;
            ...
            if ((tmp = fetch("email", feedRoot.children, true)))
                feed.author = tmp;
            feed.items = [];
            getElements("entry", feedRoot.children).forEach(function(
                item,
                index,
                list
            ) {
                ...
                if ((tmp = fetch("title", item.children))) entry.title = tmp;
                if (
                    (tmp = getElements("link", item.children, true)) &&
                    (tmp = tmp.attribs) &&
                    (tmp = tmp.href)
                )
                    entry.link = tmp;
