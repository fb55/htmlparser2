...
Parser.prototype.write = Parser.prototype.parseChunk = function(data) {
    if (this._done)
        this._handleError(
...
var parseAttributes = function(data) {
    var pos = data.search(_reAttribStart),
        attrs = {};
    ...
    while (opening !== closing) {
        ...
        if (this._contentFlags >= SpecialTags[ElementType.CDATA]) {
            if (this._tagSep === ">" && rawData.substr(-2) === "]]") {
                ...
                this._cbs.ontext(rawData + this._tagSep);
            ...
            if (elementData.charAt(0) === "/") {
                ...
                if (elementData.substr(1, 2) === "--") {
                    ...
                    if (
                        this._tagSep === ">" &&
                        elementData.substr(-2) === "]]"
                    ) {
                        if (this._cbs.oncdataend) this._cbs.oncdataend();
                        if (this._cbs.ontext)
                            this._cbs.ontext(elementData.slice(8, -2));
                    } else {
                        if (this._cbs.ontext)
                            this._cbs.ontext(elementData.substr(8));
                        this._contentFlags += SpecialTags[ElementType.CDATA];
                    }
                ...
                this._processOpenTag(
                    this._parseTagName(elementData),
                    elementData
                );
        } else {
            if (this._contentFlags !== 0) {
...
Parser.prototype._processOpenTag = function(name, data) {
    ...
    if (
        data.substr(-1) === "/" ||
        (emptyTags[name] && !this._options.xmlMode)
    ) {
