...
Parser.prototype._parseAttributes = function(data, lcNames) {
    for (var match; (match = _reAttrib.exec(data)); ) {
        this._cbs.onattribute(
            lcNames ? match[1].toLowerCase() : match[1],
...
var parseAttributes = function(data, lcNames) {
    var attrs = {};
    for (var match; (match = _reAttrib.exec(data)); ) {
        attrs[lcNames ? match[1].toLowerCase() : match[1]] =
            match[2] || match[3] || match[4] || "";
    }
    return attrs;
};
...
Parser.prototype._processOpenTag = function(data) {
    ...
    if (name in openImpliesClose) {
        var el;
        while (
            (el = this._stack[this._stack.length]) in openImpliesClose[name]
        ) {
            this._stack.pop();
    ...
    /* http://dev.w3.org/html5/html-author/#tags
         ...
         */
    if (
        (!Object.keys(attributes).length && data.substr(-1) === "/") ||
