...
var parseAttributes = function(data) {
    var pos = data.search(/\w\s/) + 1,
        attrs = {}; //Find any whitespace
    ...
    while (opening !== closing) {
        ...
        //Make sure we're not in a comment
            ...
            if (this._contentFlags >= SpecialTags.w) {
                if (this._cbs.ontext)
                    this._cbs.ontext(this._prevTagSep + rawData);
            } else {
                //The previous element was not text
                this._contentFlags += SpecialTags.w;
                if (rawData !== "" && this._cbs.ontext)
        ...
        if (elementType === ElementType.Tag) {
            ...
            if (elementName.charAt(0) === "/")
                this._processCloseTag(elementName.substr(1));
