    ...
    if (this._buffer) {
        ...
        if (this._parseState === ElementType.Tag) {
            ...
            if (name.charAt(0) === "/") this._handler.closeTag(name.substr(1));
                ...
                this._handler.openTag({
                    name: name,
                    raw: data,
                    data: data,
                    attribs: parseAttributes(data)
                });
        } else
            this._handler.writeText({
                raw: data,
                data: data,
        ...
        if (elementType === ElementType.Tag) {
            if (rawData.substring(0, 3) === "!--") {
                ...
                if (this._options.includeLocation)
                    element.location = this.getLocation(false);
                this._handler.writeDirective(element);
            } else this._processTag(elementName, elementData, tagSep, rawData);
